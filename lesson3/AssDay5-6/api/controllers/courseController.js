...
const _addstuCourse = function (req, res, student) {
    ...
    student.save(function (err, updatedStudent) {
        ...
        if (err) {
            ...
            response.status = 201;
            response.message = updatedStudent.courses;
        }
        res.status(response.status).json(response.message);
    });
}
module.exports.stuCourseAdd = function (req, res) {
    const studentId = req.params.studentId;
    ...
    Student.findById(studentId).select("courses").exec(function (err, student) {
        ...
        if (student) {
            _addstuCourse(req, res, student);
        } else {
            res.status(response.status).json(response.message);
        }
    })
}
const _updatestuCourse = function (req, res, student) {
    student.stuCourse= req.body;
...
module.exports.stuCourseUpdate = function (req, res) {
    const studentId = req.params.studentId;
    ...
    Student.findById(studentId).select("courses").exec(function (err, student) {
        ...
        if (response.status !== 204) {
            ...
            _updatestuCourse(req, res, student);
        }
    })
    
}

const _deletestuCourse = function (req, res, student) {
...
module.exports.stuCourseDelete = function (req, res) {
    // const courseId=req.params.courseId;
    const studentId = req.params.studentId;
    console.log("PUT studentId ", studentId); 
    Student.findById(studentId).exec(function (err, student) {
        ...
        if (response.status !== 204) {
            ...
            _deletestuCourse(req, res, student);
        }
    })
